...
- `MapReductionTests` tolerate the specific ArcGIS styled-basemap "Token Invalid" console error because VM E2E runs from a localhost Kestrel origin while production ArcGIS keys may be restricted to the public host. The marker-interaction test uses `basemap=empty` so it verifies GeoBlazor graphics and RuckR overlays without external basemap auth in the click path.
